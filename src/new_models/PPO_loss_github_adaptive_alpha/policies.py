...
# ---------------------------
import sys, os
sys.path.append("P:/Dokumente/3 Uni/SoSe21/Data_Compression/DataCompression/src/new_models/PPO_loss_github_adaptive_alpha")
...
class ActorCriticPolicy(BasePolicy):
    ...
    def evaluate_actions(self, obs: th.Tensor, actions: th.Tensor) -> Tuple[th.Tensor, th.Tensor, th.Tensor, th.Tensor, th.Tensor]:
        ...
        """
        latent_pi, latent_vf, latent_sde, features, features_log_var = self._get_latent(obs)
        distribution = self._get_action_dist_from_latent(latent_pi, latent_sde)
        log_prob = distribution.log_prob(actions)
        values = self.value_net(latent_vf)
        return values, log_prob, distribution.entropy(), features, features_log_var
